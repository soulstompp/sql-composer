...
pub fn command_distinct_arg(span: Span) -> IResult<Span, Option<ParsedItem<bool>>> {
    ...
    let distinct = match distinct_tag {
        Some(d) => {
            Some(ParsedItem::from_span(true, d).expect("Unable to parse bool flag from command_distinct_arg"))
        },
        None    => None
    };
...
pub fn command_all_arg(span: Span) -> IResult<Span, Option<ParsedItem<bool>>> {
    ...
    let all = match all_tag {
        Some(d) => {
            Some(ParsedItem::from_span(true, d).expect("Unable to parse bool flag from command_all_arg"))
        }
        None    => None
    };
...
pub fn of_item(span: Span) -> IResult<Span, ParsedItem<SqlCompositionAlias>> {
    ...
    let pi = ParsedItem::from_span(alias, of_name).expect("Unable to create parsed item in of_list parser");
